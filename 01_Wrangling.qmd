...
# make summary with only 1 row per nest with the highest number of _con _mod and lib females per date
beehav_sum_rep <- beehav %>%
  mutate(max_females = pmax(Num_females_lib,
                           Num_females_mod,
                           Num_females_con,
                           na.rm = TRUE)) %>%
  group_by(Nest, Date_beehav) %>%
  slice_max(max_females, n = 1, with_ties = FALSE) %>%
  ungroup()
...
beehav_sum <- beehav_sum_rep %>%
  group_by(Nest) %>%
  arrange(desc(Num_females_lib), desc(Num_females_mod), desc(Num_females_con)) %>%
  slice_head(n = 1) %>%
  ungroup()
...
KB_sol_con <- beehav_sum %>%
  ...
  distinct(Nest) %>%
  as.data.frame() # 11 if zero females is not included 
...
## Saving wrangled data
write.csv(beehav, 'Data/Behaviour/Behaviour_data_cleaned.csv')
...
length(unique(total_females_per_nest$Nest)) # 52, the extra comes from J56 with 2 painted females but the nest was never recorded 
